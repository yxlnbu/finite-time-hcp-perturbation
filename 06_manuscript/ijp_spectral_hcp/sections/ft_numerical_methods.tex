...
The literature-constrained pure-titanium baseline uses the complete 18-slip closure in Section~\ref{sec:base_model_dynamic}, a slip-gradient length $\ell_\chi=0.25~\mu\mathrm m$ and a Nye length $\ell_N=1~\mu\mathrm m$. The base history is integrated independently and then stored at 129 accepted states on $[0.5,140]~\mu\mathrm s$. At each state, the continuous constitutive Jacobian in Eq.~\eqref{eq:ft_constitutive_tangent_contract} is assembled on the recorded smooth branch. A state is rejected if a derivative probe changes that branch or if
\begin{equation}
...
near-onset candidate at $1.5~\mu\mathrm s$ and the registered terminal branch
at $140~\mu\mathrm s$. Each vector is imposed as a real periodic Fourier mode
with 16 cells per wavelength and two factor-two amplitudes. The terminal
amplitude is additionally limited so that its linearly predicted final
dimensionless norm does not exceed $10^{-3}$. The complete nonlinear periodic
...
nonretained Fourier energy and a power-partition ledger residual below
$10^{-10}$.
