...
The repository verification seed uses the complete 18-slip closure in Section~\ref{sec:base_model_dynamic}, a slip-gradient length $\ell_\chi=0.25~\mu\mathrm m$ and a Nye length $\ell_N=1~\mu\mathrm m$. Its grouped constraints are informed by titanium literature, but the card is not identified for a particular batch or specimen. The base history is integrated independently and then stored at 129 accepted states on $[0.5,140]~\mu\mathrm s$. At each state, the continuous constitutive Jacobian in Eq.~\eqref{eq:ft_constitutive_tangent_contract} is assembled on the recorded smooth branch. A state is rejected if a derivative probe changes that branch or if
\begin{equation}
...
\end{equation}
The algebraic microfield is eliminated, the generator in Eq.~\eqref{eq:dyn_generator_matrix} is assembled and the block reconstruction residual is recorded.
...
dislocation${}\times2$ and all-constitutive${}\times2$.  A shared 128-point
scrambled Sobol design, exact axes and the three registered branches seed the
bounded local searches; every objective is then evaluated on the
129-state/four-substep reference.  A compact-domain boundary maximum is
reported as such.  In particular, full/full and mechanical/mechanical maps can
select reversible high-frequency elastic waves and are not required to define
an interior localization wavelength.  Selectors that observe a constitutive
or thermal localization response must retain an interior $k^*$.
...
metric are held at their released V3 values.  The fixed-$\beta$, passive-ledger base path
...
check, not a complete Fourier calculation.  A separate audit retains every
nonnegative mode 0--8 resolvable on the 16-cell grid and repeats the replay
with one and four nonlinear midpoint substeps.  Any non-finite continuation
or more than 1\% discarded right-hand-side Fourier energy leaves the complete
nonlinear gate open.  The terminal
...
override a failed full-Fourier audit.
