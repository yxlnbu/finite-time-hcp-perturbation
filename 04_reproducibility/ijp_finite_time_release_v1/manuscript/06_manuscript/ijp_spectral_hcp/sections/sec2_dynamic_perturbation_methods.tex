...
\begin{enumerate}
\item[(H1)] There is a finite registered partition
  $t_0=\tau_0<\tau_1<\cdots<\tau_J=t_f$ such that the homogeneous base
  history is continuous on $I$, is $C^1$ on every open interval
  $(\tau_{j-1},\tau_j)$, and remains on one recorded smooth constitutive
  branch on that interval.  The tangent blocks are bounded and continuous on
  each open interval and possess finite one-sided limits at every $\tau_j$.
  Every switch has a finite one-sided event derivative and either coincident
  branch vector fields or a supplied bounded saltation matrix
  $\boldsymbol S_j$ for the retained perturbation state.
  Threshold-continuity statements are made only within a branch or with an
  explicitly declared one-sided convention.
...
\end{enumerate}
At a registered switching time, a single ordinary Jacobian is not assigned to
both sides of the surface: the left and right branch-locked tangents define the
corresponding one-sided generators.  An unregistered switch, a chattering
active set, or a noncoincident branch-vector/state jump without its
saltation/jump map lies outside H1.  If H3
fails, the Schur reduction below is disabled and the
...
$\boldsymbol A_{\mathrm{dyn}}(\cdot;k,\boldsymbol n)$ bounded and piecewise
continuous.  The value assigned at the finite set of switching times has
measure zero and does not change the propagator.  The initial-value problem
therefore has the unique absolutely continuous solution
\begin{equation}
...
More explicitly, if $t\in(\tau_{m-1},\tau_m]$ and
$\boldsymbol\Phi_j$ denotes the branch-locked propagator on the $j$th
subinterval, an identity reset and a scalar event function $h_j=0$ give the
standard saltation map \citep{KongEtAl2024}
\begin{equation}
 \boldsymbol S_j
 =\boldsymbol I_{69}
 +\frac{(\boldsymbol f_j^+-\boldsymbol f_j^-)
   (\nabla_{\!q}h_j)^{\mathsf T}}
  {\partial_t h_j+(\nabla_{\!q}h_j)^{\mathsf T}\boldsymbol f_j^-},
 \label{eq:dyn_saltation_matrix}
\end{equation}
provided the denominator is nonzero.  Composition across the registered
partition then gives
\begin{equation}
 \boldsymbol\Phi(t,t_0)
 =\boldsymbol\Phi_m(t,\tau_{m-1})\boldsymbol S_{m-1}
  \boldsymbol\Phi_{m-1}(\tau_{m-1},\tau_{m-2})\cdots
  \boldsymbol S_1\boldsymbol\Phi_1(\tau_1,t_0).
 \label{eq:dyn_piecewise_branch_product}
\end{equation}
Thus a switching history is covered only after the associated saltation
contract is stated.  For the four active-set transitions used below, the
released calculation uses the identity-saltation approximation and separately
bounds the threshold-rate jump relative to the imposed rate.  Left-, right-, and
secant-split quadratures remain numerical approximations to this same ordered
product, not alternative constitutive theories.
