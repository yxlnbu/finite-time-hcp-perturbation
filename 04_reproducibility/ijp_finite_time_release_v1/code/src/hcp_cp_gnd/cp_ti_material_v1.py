...
def _verification_kernel_root() -> Path:
    """Resolve the vendored public kernel before the private sibling checkout."""

    root = _project_root()
    if (root / "src/hcp_cp").is_dir() and (
        root / "config/verification_seed.yaml"
    ).is_file():
        return root
    sibling = root.parent / "HCP_CP_v0.1"
    if (sibling / "src/hcp_cp").is_dir() and (
        sibling / "config/verification_seed.yaml"
    ).is_file():
        return sibling
    raise FileNotFoundError(
        "neither the local vendored HCP_CP v0.1 kernel nor the sibling checkout is complete"
    )
