...
eigenspace.  It defines a quotient-space propagator only when the input
nullspace is invariant modulo the output nullspace; that condition is audited
...
class QuadraticMetricV1:
    """Hermitian PSD metric with explicit positive-eigenspace maps."""
...
def weighted_propagator_gain(
    ...
    *,
    ...
    """Largest gain restricted to the positive input/output eigenspaces.

    This quantity is a quotient-space operator norm only if
    ``Phi(ker(W_in))`` lies in ``ker(W_out)``.  Use
    :func:`metric_nullspace_invariance_audit` to test that condition.
    """
    ...
    return {
        ...
        "interpretation": "positive_metric_eigenspace_restricted_gain",
    }


def metric_nullspace_invariance_audit(
    propagator: Any,
    *,
    input_metric: QuadraticMetricV1,
    output_metric: QuadraticMetricV1,
    ...
    """Audit whether a PSD-metric propagator descends to a quotient map.

    Null vectors are normalized in the registered dimensionless coordinates
    ``q=D z``.  The relative leakage compares the output-positive response of
    input-null coordinates with the response of the complete scaled state.
    Exact quotient invariance requires zero leakage; the tolerance is solely a
    floating-point acceptance threshold.
    """
    ...
    equilibrated = (
        input_metric.coordinate_scales[:, None]
        * input_metric.matrix
        * input_metric.coordinate_scales[None, :]
    )
    values, vectors = np.linalg.eigh(equilibrated)
    scale = max(float(np.max(np.abs(values))), np.finfo(float).tiny)
    null = values <= input_metric.relative_eigenvalue_tolerance * scale
    null_coordinates = vectors[:, null]
    C_out = output_metric.unit_coordinates_from_state()
    D_in = np.diag(input_metric.coordinate_scales)
    full_response = C_out @ Phi @ D_in
    full_norm = float(np.linalg.norm(full_response, ord=2))
    if null_coordinates.shape[1] == 0:
        leakage_norm = 0.0
    else:
        leakage_norm = float(
            np.linalg.norm(full_response @ null_coordinates, ord=2)
        )
    relative_leakage = leakage_norm / max(full_norm, np.finfo(float).tiny)
    return {
        "input_nullity": int(null_coordinates.shape[1]),
        "output_positive_rank": output_metric.rank,
        "scaled_nullspace_output_spectral_norm": leakage_norm,
        "scaled_full_state_output_spectral_norm": full_norm,
        "relative_nullspace_leakage": float(relative_leakage),
        "relative_tolerance": tolerance,
        "quotient_map_well_defined_within_tolerance": bool(
            relative_leakage <= tolerance
        ),
        "normalization": "registered_dimensionless_coordinates_q=Dz",
    }
