...
DATA_FILES = (
    ...
    "05_results/ijp_reference_reoptimization_v4.json",
    "05_results/ijp_reference_reoptimization_v4_arrays.npz",
    "05_results/ijp_reference_reoptimization_v4/reference_reoptimized_winners.csv",
    "05_results/ijp_reference_reoptimization_v4/reference_optimizer_trace.csv",
    "05_results/ijp_v4_near_optimal_set_audit_v1.json",
    "05_results/ijp_v4_near_optimal_set_audit_v1.csv",
    "05_results/ijp_v4_dense_convergence_v1.json",
    "05_results/ijp_singular_vector_nonlinear_validation_v4.json",
    "05_results/ijp_singular_vector_nonlinear_validation_v4.csv",
    "05_results/ijp_singular_vector_nonlinear_validation_v4_fields.npz",
...
CODE_FILES = (
    ...
    "tools/build_ijp_reference_reoptimization_v4.py",
    "tools/audit_ijp_v4_near_optimal_set.py",
    "tools/run_ijp_v4_dense_convergence_v1.py",
    "tools/build_ijp_reference_reoptimization_v4_figure.py",
    "tools/build_ijp_v4_supplement_tables.py",
    ...
    "tests/test_ijp_v4_audits.py",
    "tests/test_lsbrp_metrics_v1.py",
    "src/hcp_cp_gnd/__init__.py",
    "src/hcp_cp_gnd/branch_audit.py",
    "src/hcp_cp_gnd/coupled_point_adapter.py",
    "src/hcp_cp_gnd/dynamic_crystal_perturbation_v1.py",
    "src/hcp_cp_gnd/dynamic_inertia_v1.py",
    "src/hcp_cp_gnd/dynamic_mechanism_causality_v1.py",
    "src/hcp_cp_gnd/evolving_work_partition_v1.py",
    "src/hcp_cp_gnd/gate01_energy_audit.py",
    "src/hcp_cp_gnd/grain_boundary.py",
    "src/hcp_cp_gnd/implicit_local.py",
    "src/hcp_cp_gnd/local_coupling.py",
    "src/hcp_cp_gnd/lsbrp_metrics_v1.py",
    "src/hcp_cp_gnd/monolithic_element.py",
    ...
    "src/hcp_cp_gnd/state_contract_v2.py",
    ...
    "src/hcp_cp_gnd/twin_damage_drx_v1.py",
    "config/cp_ti_grade1_dynamic_hcp_v1.json",
    "config/verification_seed.yaml",
)

VENDORED_HCP_CP_FILES = (
    "__init__.py",
    "crystal.py",
    "driver.py",
    "model.py",
...
MANUSCRIPT_FILES = (
    ...
    "06_manuscript/ijp_spectral_hcp/main_supplementary.pdf",
    ...
    "06_manuscript/ijp_spectral_hcp/sections/sec2_dynamic_perturbation_methods.tex",
    "06_manuscript/ijp_spectral_hcp/appendices/dyn_operator_proofs.tex",
    ...
    "06_manuscript/ijp_spectral_hcp/figures/fig01_theory_variable_framework.pdf",
    "06_manuscript/ijp_spectral_hcp/figures/fig02_classical_reduction_descriptor_verification.pdf",
    "06_manuscript/ijp_spectral_hcp/figures/fig03_evolving_base_state.pdf",
    ...
    "06_manuscript/ijp_spectral_hcp/figures/fig10_reoptimized_selector_audit.pdf",
    "06_manuscript/ijp_spectral_hcp/figures/fig11_loading_parameter_validation.pdf",
    "06_manuscript/ijp_spectral_hcp/figures/fig12_v4_singular_vector_nonlinear_validation.pdf",
...
def resolve_vendored_hcp_cp_root() -> Path:
    candidates = (
        ROOT / "src/hcp_cp",
        ROOT
        / "04_reproducibility/github_finite_time_hcp_public_v2/src/hcp_cp",
        ROOT.parent / "HCP_CP_v0.1/src/hcp_cp",
    )
    for candidate in candidates:
        if all((candidate / name).is_file() for name in VENDORED_HCP_CP_FILES):
            return candidate
    raise RuntimeError("complete vendored HCP_CP v0.1 Python kernel is unavailable")


def copy_vendored_hcp_cp() -> list[Path]:
    source_root = resolve_vendored_hcp_cp_root()
    targets = []
    for name in VENDORED_HCP_CP_FILES:
        target = RELEASE / "code/src/hcp_cp" / name
        target.parent.mkdir(parents=True, exist_ok=True)
...
def public_requirements_lock() -> str:
    ...
    lines.append("# Local package sources are included under code/src/hcp_cp and code/src/hcp_cp_gnd")
    return "\n".join(lines) + "\n"
...
def main() -> int:
    ...
    copied.extend(copy_vendored_hcp_cp())
    ...
    nonlinear_contract = required_npz_contract(
        ROOT / "05_results/ijp_singular_vector_nonlinear_validation_v4_fields.npz",
        {
    ...
    readme.write_text(
        ...
        "matched piecewise freezing, loading transfer and local sensitivity; it is "
        "retained as reconnaissance and historical provenance.\n"
        "- `ijp_reference_reoptimization_v4.*` records direct 129-state/four-substep "
        "optimization, local-stationarity gates, boundary extensions, complete V4 "
        "singular vectors and optimizer traces for all 70 contracts.\n"
        "- `ijp_v4_near_optimal_set_audit_v1.*` and `ijp_v4_dense_convergence_v1.json` "
        "record the independent sampled challenge, set-valued basin audit and "
        "513-to-1025 state recheck at the V4 coordinates.\n"
        ...
        "switching and positive-energy-subspace/nullspace-invariance audits.\n"
        "- `reference_optimizer_trace.csv` contains every retained objective evaluation "
        "for the direct-reference local-search starts.\n"
        ...
        "generated parameter and 70-contract tables, the active figures, and the "
        "comprehensive V4 revision audit.\n"
        ...
        "norm--selector robustness, direct reference-objective direction--wavenumber "
        "optimization, an anchor-assisted near-optimal-set audit and "
    ...
    gates = {
        ...
        "optimizer_trace_present": (
            RELEASE
            / "data/05_results/ijp_reference_reoptimization_v4/reference_optimizer_trace.csv"
        ).is_file(),
