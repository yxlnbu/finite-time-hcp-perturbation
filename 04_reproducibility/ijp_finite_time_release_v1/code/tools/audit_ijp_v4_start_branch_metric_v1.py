...
def branch_transitions(
    context: dict[str, Any], stop: int, spectral: Any
) -> list[dict[str, Any]]:
    points = context["points"][: stop + 1]
    storages = context["storages"][: stop + 1]
    times = np.asarray(context["times"][: stop + 1], dtype=float)
    parameters = spectral.base_model.parameters
    ...
    for interval in np.where(np.any(categories[1:] != categories[:-1], axis=1))[0]:
        ...
        for index in changed:
            ...
            if labels[index].endswith(".yield"):
                slip_index = int(labels[index][4:6]) - 1
                temperature = 0.5 * (
                    float(storages[interval].temperature_K)
                    + float(storages[interval + 1].temperature_K)
                )
                mobile_density = 0.5 * (
                    float(storages[interval].rho_mobile_m2[slip_index])
                    + float(storages[interval + 1].rho_mobile_m2[slip_index])
                )
                threshold_rate_jump = (
                    mobile_density
                    * float(parameters.burgers[slip_index])
                    * float(parameters.reference_velocity[slip_index])
                    * np.exp(
                        -float(parameters.activation_energy[slip_index])
                        / (K_B * temperature)
                    )
                )
                detail["threshold_rate_jump_s_inv"] = float(threshold_rate_jump)
                detail["secant_event_rate_Pa_s"] = float(
                    (right - left) / (times[interval + 1] - times[interval])
                )
        ...
        transitions.append(
            {
                ...
                "maximum_threshold_rate_jump_s_inv": max(
                    (
                        float(item["threshold_rate_jump_s_inv"])
                        for item in details
                        if "threshold_rate_jump_s_inv" in item
                    ),
                    default=0.0,
                ),
                "minimum_absolute_secant_event_rate_Pa_s": min(
                    (
                        abs(float(item["secant_event_rate_Pa_s"]))
                        for item in details
                        if "secant_event_rate_Pa_s" in item
                    ),
                    default=float("inf"),
                ),
            }
...
def analyze_role(
    ...
    transitions = branch_transitions(context, stop, spectral)
    imposed_shear_rate = float(
        np.median(
            np.diff(np.asarray(context["shears"][: stop + 1], dtype=float))
            / np.diff(times)
        )
    )
    ...
    energy_quotient_invariance = metric_nullspace_invariance_audit(
        physical_propagator,
        input_metric=initial_metric,
        output_metric=final_metric,
    )
    ...
    observation_quotient_invariance = metric_nullspace_invariance_audit(
        physical_propagator,
        input_metric=initial_metric,
        output_metric=observation_metric,
    )
    return {
        ...
        "imposed_shear_rate_s_inv": imposed_shear_rate,
        "maximum_threshold_rate_jump_s_inv": max(
            (
                item["maximum_threshold_rate_jump_s_inv"]
                for item in transitions
            ),
            default=0.0,
        ),
        "maximum_threshold_rate_jump_to_imposed_rate": max(
            (
                item["maximum_threshold_rate_jump_s_inv"] / imposed_shear_rate
                for item in transitions
            ),
            default=0.0,
        ),
        "minimum_absolute_secant_event_rate_Pa_s": min(
            (
                item["minimum_absolute_secant_event_rate_Pa_s"]
                for item in transitions
            ),
            default=float("inf"),
        ),
        ...
        "physical_metric_audit": {
            "classification": (
                "FIXED_DIRECTION_WAVENUMBER_POSITIVE_ENERGY_SUBSPACE_AUDIT_"
            ...
            "energy_to_energy_nullspace_invariance": energy_quotient_invariance,
            "energy_to_numerical_observation_gain": float(
                energy_to_observation["gain"]
            ),
            "energy_to_observation_nullspace_invariance": (
                observation_quotient_invariance
            ),
...
def main() -> int:
    ...
    gates = {
        ...
        <= 0.01,
        "threshold_rate_jump_below_1e_minus_12_of_imposed_rate": max(
            item["maximum_threshold_rate_jump_to_imposed_rate"]
            for item in roles.values()
        )
        <= 1.0e-12,
        "all_registered_yield_events_transverse_in_secant_audit": min(
            item["minimum_absolute_secant_event_rate_Pa_s"]
            for item in roles.values()
        )
        > 0.0,
        ...
        "energy_metric_quotient_invariance_available": all(
            item["physical_metric_audit"][
    ...
    report = {
        ...
        "interpretation": {
            ...
            "branching": (
                ...
                "effect of four registered transition intervals, while the thermally "
                "activated rate jump at each yield threshold is reported relative to "
                "the imposed shear rate to audit the identity-saltation approximation"
            ),
            "physical_metric": (
                "the recoverable-energy metric is positive semidefinite with density null "
                "directions. The audit now tests whether those null directions remain null "
                "after propagation; absent that invariance, the reported value is a gain "
                "restricted to the positive-energy input/output subspaces, not a quotient "
                "operator norm. The observation covariance is numerical, not experimental, "
                "and no energy-metric direction-wavenumber reoptimization is claimed"
            ),
        },
        "provenance": {
